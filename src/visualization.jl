...
function visualize_ma_and_persistence(input, output)
    ...
    #Slider
    sl_i = Slider(f[5, 1:4], range = 1:length(xs), startvalue = 1)
    x = sl_i.value
    ...
    two_p_ax = Axis(pg[2, 2], title = "$(pw[3]) b_2")
    tp_ax = Axis(pg[1, 1], title = L"Σ")
...
function visualize_ma(input, output, range)
    ...
    Es = output["Es"][range]
    pf = MorphoMol.Energies.get_prefactors(input["rs"], input["η"])
    Vs = pf[1] .* output["Vs"][range]
    As = pf[2] .* output["As"][range]
    Cs = pf[3] .* output["Cs"][range]
    Xs = pf[4] .* output["Xs"][range]
    OLs = input["overlap_slope"] .* output["OLs"][range]
    mol_type = input["mol_type"]    
    exp_template_centers = MorphoMol.Utilities.TWOTMVSU_EXPERIMENTAL_ASSEMBLY[mol_type]["template_centers"]
    exp_state = MorphoMol.Utilities.TWOTMVSU_EXPERIMENTAL_ASSEMBLY[mol_type]["state"]
    thetas = [MorphoMol.Utilities.average_offset_distance(exp_template_centers, input["template_centers"], exp_state, state) for state in output["states"][range]]
    xs = [i for i in 1:length(Es)];
